...
<script type="text/javascript">
	...
	function alipay(orderId,money,pname){
		location.href="alipay.jsp?oid="+orderId+"&money="+money+"&productName="+pname;
	}
</script>
</head>
	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="${pageContext.request.contextPath }/order.do?method=show" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
				<c:if test="${order.ostate eq 1 }">&nbsp;&nbsp;
					<button type="button" onclick="alipay('${order.oid}','${order.ocount}','订单${order.oid}')" class="btn btn-success btn-sm" >支付宝支付</button>
				</c:if>
			</td>
		</tr>
	</table>
